...
STATIC bool check_boot_py()
{
	spiffs_user_mount_t* vfs_spiffs = &spiffs_user_mount_handle;
	printf("[MAIXPY]:Spiffs Write init file\n");
	spiffs_file fd;
	fd=SPIFFS_open(&vfs_spiffs->fs,"boot.py", SPIFFS_CREAT | SPIFFS_TRUNC | SPIFFS_RDWR, 0);
	if(fd != -1){
		s32_t ls_res = SPIFFS_lseek(&vfs_spiffs->fs, fd,0,0);
		if(!ls_res){
			s32_t w_res = SPIFFS_write(&vfs_spiffs->fs, fd,boot_py,sizeof(boot_py) - 1);
			if(w_res <= 0){
			}else{
