...
int avi_init(uint8_t* buf, uint32_t size, avi_t* avi)
{
 	...
 	if(strh_header->stream_type==AVI_VIDS_STREAM)
	{
		...
		{			
			...
			avi->audio_type=wav_header->format_tag;
		}
	...
	{
		buf_start+=offset;
		offset=avi_srarch_id(buf_start,size,avi->audio_flag);
		if(offset==0)return AVI_STATUS_ERR_STREAM;
		buf_start+=offset+4;
		avi->audio_buf_size=*((uint32_t*)buf_start);
	}		
	return res;
}
